...
namespace CrypticCabinet.Photon.Colocation
{
    ...
    public class PhotonNetworkMessenger : NetworkBehaviour, INetworkMessenger
    {
        ...
        {
            Debug.Log($"SendMessageUsingOculusId called: eventCode: {eventCode}, oculusId: {oculusId}");
            if (m_idDictionary == null)
            {
                Debug.LogError(
                    "NetcodeGameObjectsMessenger doesn't have a dictionary to go from oculus id to network id");
                return;
            }
        ...
        public void UnregisterEventCallback(byte eventCode) => _ = m_callbackDictionary.Remove(eventCode);

        public override void Spawned() => NetworkAdapter.NetworkMessenger = this;
        ...
        public void Init(PhotonPlayerIDDictionary idDictionary) => m_idDictionary = idDictionary;
            ...
            NetworkBool anchorFlowSucceeded, RpcInfo info = default)
        {
            ...
            FindRPCToCallClientRPC(
                playerRef, eventCode, oculusIdAnchorOwner, oculusIdAnchorRequester, headsetIdRequester, uuid,
                anchorFlowSucceeded);
        }
            ...
            string uuid, NetworkBool anchorFlowSucceeded)
        {
            ...
            var data = new ShareAndLocalizeParams(
                oculusIdAnchorOwner, oculusIdAnchorRequester, headsetIdRequester, uuid)
            {
